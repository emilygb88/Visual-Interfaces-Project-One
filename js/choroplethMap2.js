class ChoroplethMap2 {
    ...
    mapNumericToLabel(attribute, value) {
        ...
        // For other attributes, return value unchanged
        return value;
    }
     ...
     * @param {Object} newGeoData - New data for updating the visualization
     * @param {String} selectedAttribute - The selected attribute being visualized
     * @param {String} selectedAttributeLabel - Label for the selected attribute
     */
    updateVis(newGeoData, selectedAttribute, selectedAttributeLabel) {
        ...
        vis.counties
            .on('mouseover', function (event, d) {
                const countyData = d.properties;
                // Get the attribute value and label
                const attributeValue = d.properties.pop; // Update this with the actual attribute value you want to display
                const attributeLabel = vis.mapNumericToLabel(selectedAttribute, attributeValue); // Use the mapNumericToLabel method
                ...
                d3.select('#tooltip-map')
                    ...
                    .html(`
                        <div><strong>County:</strong> ${countyData.name}</div>
                        <div><strong>${selectedAttributeLabel}:</strong> ${attributeLabel}</div> 
                    `)
